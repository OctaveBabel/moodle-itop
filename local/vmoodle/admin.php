...
// Printing headers.
$PAGE->set_url(new moodle_url('/local/vmoodle/admin.php'));
...
switch ($action) {
    case 'enablecommands':
        ...
        $commands = optional_param('commands', null, PARAM_TEXT);
        if (is_null($commands)) {
...
$plugins = core_plugin_manager::instance()->get_plugins_of_type('vmoodleadminset');
foreach($plugins as $key => $plugin) {
    if (!$DB->get_record('config_plugins', array('plugin' => $plugin->type.'_'.$plugin->name, 'name' => 'version'))) {
        unset($plugins[$key]);
    }
}
...
foreach ($plugins as $plugin) {
    $table->data[] = array($plugin->type.'_'.$plugin->name, $OUTPUT->single_button(new moodle_url('admin.php', array('action' => 'uninstallplugin', 'plugin' => $plugin->type.'_'.$plugin->name)), get_string('uninstall', 'local_vmoodle'), 'get'));
}
