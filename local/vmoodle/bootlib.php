...
function vmoodle_get_hostname() {
    ...
    try {
        $server_httphost = explode(':', $_SERVER['HTTP_HOST']);
        if(count($server_httphost) < 1 || empty($server_httphost[0]))
            throw new Exception('[local/vmoodle/bootlib.php] - vmoodle_get_hostname() : Error on HTTP_HOST header.');
        
        $CFG->vmoodleroot = "{$protocol}://".$server_httphost[0];
        $CFG->vmoodlename = $server_httphost[0];
    ...
    if ($CFG->vmoodleroot != $CFG->wwwroot) {
        if($CFG->vmasterdbtype == 'sqlsrv') {
            ...
            try {
                ...
                if (!is_bool($result)) { // true/false resources cannot be freed
        ...
        else if ($CFG->vmasterdbtype == 'mysql') {
            $vmaster = new StdClass();
            $vmaster->vdbtype = $CFG->vmasterdbtype;
            $vmaster->vdbhost = $CFG->vmasterdbhost;
            $vmaster->vdblogin = $CFG->vmasterdblogin;
...
function vmoodle_make_connection(&$vmoodle, $binddb = false) {
    ...
    if ($vmoodle->vdbtype == 'sqlsrv') {
        $sqlsrv_side_cnx = @sqlsrv_connect($vmoodle->vdbhost, array
        (
            'UID' => $vmoodle->vdblogin,
            'PWD' => $vmoodle->vdbpass,
            'Database' => $vmoodle->vdbname,
            'CharacterSet' => 'UTF-8',
        ...
        if (!$sqlsrv_side_cnx) {
            throw new Exception ("[local/vmoodle/bootlib.php] - VMoodle_make_connection() : Server $vmoodle->vdbhost unreachable\n");
        }
        return $sqlsrv_side_cnx;
    }
    else if ($vmoodle->vdbtype == 'mysql') {
        ...
        if ($binddb) {
            if (!mysql_select_db($vmoodle->vdbname, $mysql_side_cnx)){
                throw new Exception ("[local/vmoodle/bootlib.php] - VMoodle_make_connection : Database not found");
            }
        }
        ...
        if (!$mysql_side_cnx) {
            throw new Exception ("[local/vmoodle/bootlib.php] - VMoodle_make_connection : Server {$vmoodle->vdblogin}@{$vmoodle->vdbhost} unreachable");
        }
        if ($binddb) {
    ...
    throw new Exception ("[local/vmoodle/bootlib.php] - VMoodle_make_connection() : Database not supported<br/>");
}
