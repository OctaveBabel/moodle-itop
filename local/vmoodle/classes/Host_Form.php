...
class Host_Form extends \moodleform {
    ...
    */
    ...
    public function definition() {
        ...
        // Database type.
        $dbtypearray = array('mysqli' => 'MySQL', 'postgres' => 'PostgreSQL', 'sqlsrv' => 'SQLServer');
     ...
     * @param object $vmoodle The database data to check.
     * @return bool If TRUE, the chosen database name is already used, else FALSE.
     */
    private function isEqualToAnotherDatabaseName($vmoodle) {
        global $DB;

        if($vmoodle['vdbtype'] == 'sqlsrv') {
            $vdbs = $DB->get_records_sql('EXEC sp_databases');
            if (!empty($vdbs)) {
                // Retrieves all the databases names.
                $vdbnames = array();
                foreach ($vdbs as $vdb) {
                    $vdbnames[] = $vdb->database_name;
                }
                return in_array($vmoodle['vdbname'], $vdbnames);
            }
        } else {
