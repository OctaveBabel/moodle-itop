...
// now get cli options
list($options, $unrecognized) = cli_get_params(array('help' => false, 'mode' => 'cli', 'trace' => false),
                                               array('h' => 'help', 'm' => 'mode', 't' => 'trace'));
...
-m, --mode            Running mode : cli or web (default cli)
-t, --trace           Log script output into {dataroot}/vcrontrace.log

Example:
\$sudo -u www-data /usr/bin/php local/vmoodle/cli/vcron.php
";
...
// choose how individual cron are launched
$VCRON->ACTIVATION = 'cli';
if(isset($options['mode']) && $options['mode'] === 'web') {
    $VCRON->ACTIVATION = 'web';
}
...
$VCRON->TRACE_ENABLE = $options['trace'];           // enables tracing
...
function exec_vhost_cron($vhost) {
    ...
    $cmd = $phpcmd.' "'.$CFG->dirroot.'/local/vmoodle/cli/cron.php" --host='.$vhost->vhostname;
