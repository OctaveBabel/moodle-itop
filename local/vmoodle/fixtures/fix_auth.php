...
foreach($vmoodles as $vm) {
    if($CFG->dbtype == 'sqlsrv') {
        $sql = "UPDATE [{$vm->vdbname}].[{$vm->vdbprefix}config] SET value = value+',mnet' WHERE name = 'auth'";
    } else {
        $sql = "UPDATE `{$vm->vdbname}`.`{$vm->vdbprefix}config` SET value = CONCAT(value, ',mnet') WHERE name = 'auth'";
    }
    
    $DB->execute($sql);

    if($CFG->dbtype == 'sqlsrv') {
        $sql = "UPDATE [{$vm->vdbname}].[{$vm->vdbprefix}config] SET value = REPLACE(value, ',mnet,mnet', ',mnet') WHERE name = 'auth'";
    } else {
        $sql = "UPDATE `{$vm->vdbname}`.`{$vm->vdbprefix}config` SET value = REPLACE(value, ',mnet,mnet', ',mnet') WHERE name = 'auth'";
    }
    $DB->execute($sql);
