...
if ($vmoodles) {
    ...
    $strsize = get_string('size', 'local_vmoodle');
    ...
    $table->head = array('', "<b>$strname</b>","<b>$strhost</b>","<b>$strstatus</b>","<b>$strsize</b>","<b>$strmnet</b>","<b>$strcrons</b>","<b>$strlastcron</b>","<b>$strlastcrongap</b>","<b>$strcmds</b>");
    $table->align = array ('CENTER', 'LEFT', 'LEFT', 'CENTER', 'CENTER', 'CENTER', 'CENTER', 'CENTER', 'CENTER', 'CENTER');
    $table->size = array('2%', '20%', '30%', '6%', '6%', '6%', '6%', '8%', '8%', '8%');
    $table->width = '98%';
    ...
    foreach ($vmoodles as $vmoodle) {
        ...
        if ($vmoodle->enabled == 1) {
            ...
            $vmoodlecmd .= ' <a href="'.$fulldeleteurl.'" onclick="return confirm(\''.get_string('confirmfulldelete', 'local_vmoodle').'\');"><img src="'.$OUTPUT->pix_url('t/delete').'" title="'.get_string('fulldeletehost', 'local_vmoodle').'" /></a>';
        }
        $snapshoturl = new moodle_url('/local/vmoodle/view.php', array('view' => 'management', 'what' => 'snapshot', 'wwwroot' => $vmoodle->vhostname));
        ...
        if (!isset($vmoodle->vdatasize) || empty($vmoodle->vdatasize)) {
            $strsize = 0;
        } else {
            $strsize = round($vmoodle->vdatasize/1000000, 2);
        }
        ...
        $table->data[] = array($vmoodlecheck, $vmoodlelnk, $hostlnk, $vmoodlestatus, $strsize, $strmnet, $vmoodle->croncount, userdate($vmoodle->lastcron), $crongap, $vmoodlecmd);
    }
